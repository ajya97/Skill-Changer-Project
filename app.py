...
def home():
    ...
    user = skdb.query.filter_by(user_name=username, Password=pas).first()
    if user:
...
def login():
    if request.method == 'POST':
        ...
        user = skdb.query.filter_by(user_name=user_name, Password=password).first()
...
def register_user():
    if(request.method=='POST'):
        ...
        entry = skdb(user_name = userid, Name = name, Email = email, Password= password, Ph_num = phone)
